...
class ListVehiclePage extends StatelessWidget {
  const ListVehiclePage({Key? key}) : super(key: key);
  ...
  Future<void> manageCreateEdit(BuildContext context, FORM_ACTION action,
      ListVehicleViewModel viewModel, Vehicle? data) async {
    var response = await fluent.showDialog<Vehicle?>(
      context: context,
      builder: (context) {
  ...
  Future<void> _showLoading(BuildContext context) async {
    await fluent.showDialog<Vehicle?>(
      context: context,
      builder: (context) {
        return ContentDialog(
